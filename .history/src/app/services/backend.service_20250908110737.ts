...
export class BackendService {
  ...
  checkBackendStatus(): Observable<{online: boolean, message: string}> {
    ...
    // A URL real será http://localhost:8080/api/status, mas o proxy intercepta
    // chamadas para /api/* e redireciona para o backend
    const url = `${this.apiUrl}/status`;
    console.log('Verificando backend em (via proxy):', url);
    console.log('Isso será redirecionado para: http://localhost:8080/api/status');
